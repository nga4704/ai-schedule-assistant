...
function mapFirestoreTaskToCalendar(task: any): Task {
  const start: Date = task.startTime?.toDate?.() ?? new Date(task.startTime);
  return {
    id: task.id,
    title: task.title,
...
export default function CalendarScreen() {
  ...
  useFocusEffect(
    React.useCallback(() => {
      ...
      async function load() {
        ...
        const rangeEnd = new Date(selectedDate.getFullYear(), selectedDate.getMonth() + 1, 0);

        const expanded = firestoreTasks.flatMap(t => expandTaskByRepeat(t, rangeStart, rangeEnd));
  ...
  const tasksOfDay = tasks.filter(t => t.date === formatDateLocal(selectedDate));
    ...
    <View style={styles.wrapper}>
      <ScrollView style={styles.container} showsVerticalScrollIndicator={false}>
        ...
        {!loading && viewMode === "day" && (
          <View style={{ marginBottom: 16 }}>
            <View style={styles.rowBetween}>
              <TouchableOpacity onPress={() => setSelectedDate(new Date(selectedDate.getFullYear(), selectedDate.getMonth(), selectedDate.getDate() - 1))} style={styles.navBtn}>
                <Feather name="chevron-left" size={22} />
              </TouchableOpacity>

              <TouchableOpacity onPress={() => setShowPicker(true)}>
                <Text style={{ fontWeight: "800" }}>{formatDateLocal(selectedDate)}</Text>
              </TouchableOpacity>
        ...
        {!loading && viewMode === "week" && (
          <View style={{ marginBottom: 16 }}>
            <View style={styles.rowBetween}>
              <TouchableOpacity onPress={() => setSelectedDate(new Date(selectedDate.getFullYear(), selectedDate.getMonth(), selectedDate.getDate() - 7))} style={styles.navBtn}>
                <Feather name="chevron-left" size={22} />
              </TouchableOpacity>

              <TouchableOpacity onPress={() => setShowPicker(true)}>
                <Text style={{ fontWeight: "800" }}>{formatDateLocal(weekDates[0])} — {formatDateLocal(weekDates[6])}</Text>
              </TouchableOpacity>
        ...
        {!loading && viewMode === "month" && (
          <View style={{ marginBottom: 16 }}>
            <View style={styles.rowBetween}>
              <TouchableOpacity onPress={() => setSelectedDate(new Date(selectedDate.getFullYear(), selectedDate.getMonth() - 1, 1))} style={styles.navBtn}>
                <Feather name="chevron-left" size={22} />
              </TouchableOpacity>

              <TouchableOpacity onPress={() => setShowPicker(true)}>
                <Text style={{ fontWeight: "800" }}>{formatMonthTitle(selectedDate)}</Text>
              </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  modeSwitch: { flexDirection: "row", backgroundColor: Colors.card, borderRadius: 16, padding: 6, marginBottom: 24 },
  ...
  navBtn: { width: 42, height: 42, borderRadius: 21, backgroundColor: Colors.card, justifyContent: "center", alignItems: "center" },
  fab: { position: "absolute", right: 24, bottom: 32, width: 56, height: 56, borderRadius: 28, backgroundColor: Colors.primary, justifyContent: "center", alignItems: "center", elevation: 6 },
});
